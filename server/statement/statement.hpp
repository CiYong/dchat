...
class BasicCommand {
public:
    virtual RawCommand to_command() { return {}; }
};
...
template<typename T>
class BaseStatement : public ToQuery, public BasicCommand {
public:
    using Table = T;
...
//        auto reply = conn_.execute(query);
        ...
        auto command = to_command();
        auto reply = conn_.execute(command.command.c_str(), "%s %s %s", command.key.c_str(), command.value.c_str());
...
public:
    ...
    inline virtual RawCommand to_command() {
        RedisSerializer ser;
        auto pair = ser.serialize(_arg);
